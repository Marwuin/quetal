...
open class MobilePlayerViewController: MPMoviePlayerViewController {
  ...
  private func initializeControlsView() {
    ...
    if let fav = getViewForElementWithIdentifier("fav") as? Button {

        if item!.isFavorite! {
            fav.setImage(#imageLiteral(resourceName: "fav"), for: UIControlState.normal)
        }
        else {
            fav.setImage(#imageLiteral(resourceName: "unfav"), for: UIControlState.normal)
        }
        ...
        fav.addCallback(
            callback: { [weak self] in
                ...
                slf.delegate?.didPressFavButton(sender: fav)
